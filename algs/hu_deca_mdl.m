function [varargout]=hu_deca_mdl(varargin)
%
%% -------------- DECA Sintax -------------- 
%
        ...
        % no record of the parameters evolution
         ...
         while Kmod >= Kmod_min
             ...
             while ~stop_while  & (iter < Max_iter) 
                   ...
                   [A_est] = hu_dusal(Y,p, 'dir_pars', dir_pars ,'spherize', 'no','MM_ITERS',5,'MU',1 ,'M0',A_est,'verbose',0);
                   ...
                   if vefig
                      ...
                      figure(1)
                        subplot(333)
                          ...
                          hold off
                          drawnow
                        subplot(331);plot(L_global);
                        eval(['title(' num2str(L_variat) ');']);
                        subplot(334);plot(e_global)
                        subplot(336)
                          regions=beta>th;
                          regions=regions*(1:size(regions,2))';
                          imagesc(reshape(regions(:,1),[N_LINES N_COLUMNS,]));colormap([0 0 0; ycolor(1:Kmod,:)])
                          title('Image Regions')
                          xlabel('columns')
                          ylabel('lines')
                          freezeColors 
                   ...
                   if verb, [e_hat;nan*ones(size(e_hat)); a_hat], end;
