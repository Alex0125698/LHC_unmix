function [M,Sest,Up,my,sing_values] = mvsa(Y,p,varargin)
...
for k = 1:MMiters
    ...
    elseif exitflag == 0 % Number of iterations exceeded options.MaxIter.
        ...
        %if verbose
            fprintf('\n iterations exceeded: iter = %d, f0_quad = %2.4f, f_quad = %2.4f \n',...
                k, f0_quad, f_quad)
        %end
...
%         end
    ...
    % termination test
    if energy_decreasing
        if abs((f_val_back-f_val)/f_val) < tol_f && ~throtle
            if verbose
