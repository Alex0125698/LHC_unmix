function [M,Z,Up,my,sing_values] = hu_sisal(Y,p,varargin)
...
if M == 0
    % Initialize with VCA
    Mvca = hu_vca(Y,'Endmembers',p,'verbose','off','SNR',1);
    M = Mvca;
    % expand Q
    Ym = mean(M,2);
    Ym = repmat(Ym,1,p);
...
%%
% ---------------------------------------------------------------
%  Main body- sequence of quadratic-hinge subproblems (MMiters)
%----------------------------------------------------------------

% initializations
Z = Q*Y;
Bk = 0*Z;   % Lagrange multipliers
...
for k = 1:MMiters
    ...
    if verbose == 1
        ...
        fprintf('\n iter = %d, simplex volume (estimated M) = %4f  \n', k, abs(det(M)))
    end

    
    %  in the last quadratic-hinge subproblem, we may set more AL_iters
    if k==MMiters
        AL_iters = 4;
    end
    ...
    % initial function values (true and quadratic)
        ...
        f0_quad =   tau*sum(sum(hinge(Q*Y)));
    
    % run  sets of AL_iters  until the  the quadractic energy decreases 
    while 1 > 0
        q = Q(:);
        % initial function values (true and quadratic)
%          f0_val = -log(abs(det(Q)))+ tau*sum(sum(hinge(Q*Y)));
%          f0_quad = (q-q0)'*g+1/2*(q-q0)'*H*(q-q0) + tau*sum(sum(hinge(Q*Y)));
        for i=2:AL_iters
            ...
            %-------------------------------------------
            % update Bk  (Lagrange multipliers)
            %-------------------------------------------
            Bk = Bk - (Q*Y-Z);
            if verbose == 3 ||  verbose == 4
                fprintf('\n ||Q*Y-Z|| = %4f \n',norm(Q*Y-Z,'fro'))
        ...
        if verbose == 3 ||  verbose == 4
            fprintf('\n MMiter = %d, AL_iter, = % d,  f0_quad = %2.4f, f_quad = %2.4f,  f0_val  = %2.4f  \n',...
                k,i, f0_quad,f_quad, f0_val)
        end
