function [M,Up,Z,my,sing_values] = hu_dusal(Y,p,varargin)

%% -------------- DUSAL Sintax -------------- 
%
% Syntax:
%        [M,Up,my,sing_values] = DUSAL(Y,p,varargin)
%
% Dirichlet mixture unmixing via split augmented Lagrangian (DUSAL)
% This code is part of the DECA method and is partially based on SISAL.
%
%
%% -------------- Required inputs -------------- 
%
...
%
%     DUSAL assumes that y belongs to an affine space. It may happen,
...
%                 2 - display figures
%                 3 - display DUSAL information 
%                 4 - display DUSAL information and figures
%
%
%
%
...
%
% DUSAL is distributed under the terms of
...
if M == 0
    % Initialize with VCA
    Mvca = hu_vca(Y,'Endmembers',p,'verbose','off');
    M = Mvca;
    % expand Q
    Ym = mean(M,2);
    Ym = repmat(Ym,1,p);
...
for k = 1:MMiters
    ...
    % f0_val = -log(abs(det(Q0)))-dir_par'*sum(log(Q0*Y),2);;
    ...
    while 1 > 0
        q = Q(:);
        % initial function values (true and quadratic)
        f0_val = -log(abs(det(Q0)))-(dir_pars-1)'*sum(log(Q0*Y),2)/N;
        f0_quad = (q-q0)'*g+1/2*(q-q0)'*H*(q-q0) -(dir_pars-1)'*sum(log(Q0*Y),2)/N;
        for i=2:AL_iters
            %-------------------------------------------
            ...
            %-------------------------------------------
            % solve with respect to Z
            %-------------------------------------------
            %Z = soft_neg(Q*Y -Bk,tau/mu);
            
            Z1 = Q*Y -Bk;
            if prod(size(dir_pars))==p  % one mode
               Z2 = Z1.^2+4*repmat(dir_pars-1,1,N)/mu/N;
            else                         % multiple modes (Mixture of Dirichlets)
               Z2 = Z1.^2+4*(dir_pars-1)/mu/N; 
            end
        ...
        f_quad = (q-q0)'*g+1/2*(q-q0)'*H*(q-q0) -(dir_pars-1)'*sum(log(Q*Y),2)/N;
        if verbose == 3 ||  verbose == 4
            fprintf('\n MMiter = %d, AL_iter, = % d,  f0 = %2.4f, f_quad = %2.4f,  \n',...
                k,i, f0_quad,f_quad)
        end
%         f_val = -log(abs(det(Q)))-(dir_pars-1)'*sum(log(Q*Y),2)/N;
